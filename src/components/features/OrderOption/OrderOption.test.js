...
describe('Component OrderOption', () => {
  ...
  it('should render correct name', () => {
    ...
    const type = 'text';
    const component = shallow(<OrderOption name={name} type={type}/>);
    expect(component.find('.component h3').text()).toEqual(name);
  });
      ...
      switch (type) {
        ...
        case 'icons': {
          it('contains icons', () => {
            ...
            const emptyIcon = icon.find('[name="times-circle"]').length;
            expect(emptyIcon).toBe(1);
            const icons = icon.not('[name="times-circle"]');
          ...
          it('should run setOrderOption function on click', () => {
            renderedSubcomponent.find('div').at(3).simulate('click', {currentTarget: {value: testValue}});
            expect(mockSetOrderOption).toBeCalledTimes(3);
            expect(mockSetOrderOption).toBeCalledWith({[mockProps.id]: testValue});
          });
          break;
        }
        ...
        case 'text': {
          it('contains input', () => {
            ...
            expect(select.length).toBe(1);
            const input = select.find('input').not('[value=""]');
            expect(input.length).toBe(1);
